...
from aiovelib.s2 import S2ServerItem #type:ignore
...
from s2python.s2_parser import S2Parser, TYPE_TO_MESSAGE_CLASS
from s2python.s2_validation_error import S2ValidationError
from s2python.message import S2Message
from s2python.version import S2_VERSION

from s2python.s2_message_handlers import MessageHandlers #type:ignore
from s2python.s2_asset_details import AssetDetails #type:ignore
...
class S2ResourceManagerItem(S2ServerItem):
    ...
    def __init__(self, path,
        ...
        self._handlers.register_handler(HandshakeResponse, self.handle_handshake_response_as_rm)
         ...
         #FIXME: Temporary workaround, Add missing OMBC.Instruction to s2_parser once during init.
        global TYPE_TO_MESSAGE_CLASS
        TYPE_TO_MESSAGE_CLASS["OMBC.Instruction"] = OMBCInstruction

    async def _create_connection(self, client_id: str, keep_alive_interval: int):
        logger.info("Creating Keep-Alive listener with interval {}".format(keep_alive_interval))
