...
class TestHubSystem(TestSystemCalcBase):
	...
	def test_control_vedirect_solarcharger_bms_ess_feedback_generator(self):
		# When feedback is allowed we do not limit MPPTs, unless there is a generator connecte to the active
		# AC-In.
		...
		self._monitor.add_value('com.victronenergy.vebus.ttyO1', '/Hub/ChargeVoltage', 58.3)
		self._monitor.add_value('com.victronenergy.settings', '/Settings/CGwacs/OvervoltageFeedIn', 1)
		self._monitor.add_value('com.victronenergy.settings', '/Settings/SystemSetup/AcInput1', 2)  # genset
		self._add_device('com.victronenergy.solarcharger.ttyO2', {
			'/State': 1,
			'/Settings/ChargeCurrentLimit': 100,
			'/Link/NetworkMode': 0,
