...
class Hub1Bridge(SystemCalcDelegate):
	...
	ServiceItems = collections.namedtuple('ServiceItems', ['actual', 'max', 'limit'])
	...
	def _maximize_charge_current(self, vebus_path, bms_max_charge_current, vedirect_chargers):
		...
		try:
			self._dbusmonitor.set_value(vebus_path, '/Dc/0/MaxChargeCurrent',
				dbus.Double(bms_max_charge_current, variant_level=1))
		except dbus.exceptions.DBusException:
			logging.debug(traceback.format_exc())
	...
	def _distribute_max_charge_current(self, vebus_path, bms_max_charge_current, vedirect_chargers):
		...
		all_chargers = []
		for service in vedirect_chargers:
			charger = Hub1Bridge.ServiceItems(
				actual=self._dbusmonitor.get_item(service, '/Dc/0/Current'),
				max=self._dbusmonitor.get_item(service, '/Link/ChargeCurrent'),
				limit=self._dbusmonitor.get_item(service, '/Settings/ChargeCurrentLimit'))
			charge_current = charger.actual.get_value()
			solar_charger_current = safeadd(solar_charger_current, charge_current)
			max_charge_current = charger.max.get_value()
		...
		if vebus_path is not None:
			...
			bol_item = self._dbusmonitor.get_item(vebus_path, '/BatteryOperationalLimits/MaxChargeVoltage')
			...
			if bol_item.exists:
				...
				try:
					self._dbusmonitor.set_value(vebus_path, '/Dc/0/MaxChargeCurrent',
						dbus.Double(vebus_max_charge_current, variant_level=1))
				except dbus.exceptions.DBusException:
					logging.debug(traceback.format_exc())
		...
		if extra_solar_charger_max_current >= 0:
			extra_solar_charger_max_current = \
				Hub1Bridge._distribute_currents(upscalable_chargers, extra_solar_charger_max_current)
			# Scale up the max charge current to prevent the MPPT to be categorized as non static the next
			# time.
			Hub1Bridge._distribute_currents(static_chargers, extra_solar_charger_max_current,
			...
			for charger in all_chargers:
				try:
					...
					charger.max.set_value(dbus.Double(max_charge_current, variant_level=1))
				except dbus.exceptions.DBusException:
					logging.debug(traceback.format_exc())
	...
	def _distribute_currents(chargers, increment, scale=1.0):
		...
		for charger in chargers:
			actual_currents.append(max(0, charger.actual.get_value()))
			limits.append(charger.limit.get_value())
		max_currents = Hub1Bridge.distribute(actual_currents, limits, increment)
		i = 0
		for charger in chargers:
			try:
				charger.max.set_value(dbus.Double(scale * max_currents[i], variant_level=1))
