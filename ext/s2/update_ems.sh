...
if [ "$num_mock" -eq 1 ]; then
    ...
    wget -P /tmp https://github.com/victronenergy/dbus-shelly/archive/refs/heads/dmanner/ems-compatibility-fixes.zip -O "tmp/dbus-shelly.zip" > /dev/null 2>&1

    echo " > Extracting shelly-dbus..."
    unzip /tmp/dbus-shelly.zip -d /tmp/dbus-shelly > /dev/null 2>&1
    
    echo " > Copying shelly-dbus..."
    cp -Rf /tmp/dbus-shelly/dbus-shelly-dmanner-ems-compatibility-fixes/* /opt/victronenergy/dbus-shelly
