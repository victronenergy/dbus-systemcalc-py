...
try:
    # Get tzdata's internal path to zoneinfo data
    tz_path = importlib.resources.files("tzdata").joinpath("zoneinfo")
    if not os.path.exists(tz_path):
        raise FileNotFoundError("tzdata is installed but zoneinfo files are missing")

    # Override the zoneinfo search path
    zoneinfo.TZPATH = [str(tz_path)]  # This is the key patch
except Exception as e:
    print(f"Failed to patch zoneinfo to use tzdata: {e}")
...
class S2ResourceManagerItem(S2ServerItem):
    ...
    async def _connect_and_run(self) -> None:
        ...
        background_tasks = [
            # self._runningloop.create_task(self._receive_messages()),
