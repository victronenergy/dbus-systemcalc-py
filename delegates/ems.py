...
class S2RMDelegate():
	...
	def _s2_on_power_measurement(self, message:PowerMeasurement):
		...
		#logger_debug_proxy("Received power emasurement from {}: {}".format(self.unique_identifier, message))
		
		#TODO: During the first measurements, we should start to track the average reporting interval of a consumer.
		#      Consumers may stop reporting / working, but still answer the keep alive. Need to additionaly detect
		#      stale consumers and drop them. 
		...
		#only accept power reports, when the consumer is actually actively EMS controlled. Else we don't care and assume 0. 
		#FIXME: Exception: If a consumer is stuck in Transition to off, it is under EMS Control, so power reports shall be accepted.
	...
	def _ombc_self_assign_overhead(self, overhead:SolarOverhead)->SolarOverhead:
		...
		logger_debug_proxy("Eligible States for consumer {}: {}".format(self.unique_identifier, 
															[mode.diagnostic_label for mode in eligible_operation_modes]))

		if len(eligible_operation_modes) == 0:
			logger.error("No valid operationmodes to choose from.")
			return overhead
		
		#this is our last resort.
		forced_state = eligible_operation_modes[len(eligible_operation_modes) -1]
		logger_debug_proxy("Forced State for consumer {}: {}".format(self.unique_identifier, forced_state.diagnostic_label))
		...
		for opm in eligible_operation_modes:
			...
			for pr in opm.power_ranges:
				...
				if not claim_success:
					...
					logger_debug_proxy("Operation Mode not eligible: '{}' on {} due to missing availability on commodity: {}".format(opm.diagnostic_label, self.unique_identifier, pr.commodity_quantity))
					overhead.rollback()
					break
			
			if overhead.transaction_running:
				#Managed to verify all power ranges and transaction still running? This mode is eligible! 
				logger_debug_proxy("Operation Mode selected: '{}' on {}".format(opm.diagnostic_label, self.unique_identifier))

				old_power_claim = self.power_claim
				new_power_claim = overhead.comit()

				logger_debug_proxy("Power-Claim: {}".format(new_power_claim))

				if (old_power_claim is not None and old_power_claim.total != new_power_claim.total):
					logger_debug_proxy("Power Claim changed from {}W to {}W. Overhead now is: {}W".format(old_power_claim.total, new_power_claim.total, overhead.power.total))
				...
				if self._ombc_check_timer_block() > 0:
					...
					self.is_active_ems_control = self.power_request.total > 0
					#logger_debug_proxy("Flaging consumer to be under EMS control: {} for {}".format(self.is_active_EMS_control, self.unique_identifier))
				break
...
class EMS(SystemCalcDelegate):
	...
	def set_sources(self, dbusmonitor, settings, dbusservice):
		...
		for l in [1,2,3]:
			self.counter_primary.by_phase[l] = settings["ems_primary_l{}_forward".format(l)]
			self.counter_secondary.by_phase[l] = settings["ems_secondary_l{}_forward".format(l)]
		...
		self._dbusservice.add_path('/Ems/SystemType', value=0, gettextcallback=lambda p, v: SystemType(v))

		for l in [1,2,3]:
			self._dbusservice.add_path('/Ems/PrimaryConsumer/Ac/L{}/Power'.format(l), value=None)
			self._dbusservice.add_path('/Ems/PrimaryConsumer/Ac/L{}/Energy/Forward'.format(l), value=self.counter_primary.by_phase[l])
			self._dbusservice.add_path('/Ems/SecondaryConsumer/Ac/L{}/Power'.format(l), value=None)
			self._dbusservice.add_path('/Ems/SecondaryConsumer/Ac/L{}/Energy/Forward'.format(l), value=self.counter_secondary.by_phase[l])
		
		self._dbusservice.add_path('/Ems/PrimaryConsumer/Ac/Power', value=None)
		self._dbusservice.add_path('/Ems/PrimaryConsumer/Ac/Energy/Forward', value=self.counter_primary.total)
		self._dbusservice.add_path('/Ems/SecondaryConsumer/Ac/Power', value=None)
		self._dbusservice.add_path('/Ems/SecondaryConsumer/Ac/Energy/Forward', value=self.counter_secondary.total)
	...
	def get_settings(self):
		...
		settings = [
			...
			("ems_primary_l1_forward", path + "/Energy/Primary/L1/Forward", 0.0, 0.0, 999999.9),
			("ems_primary_l2_forward", path + "/Energy/Primary/L2/Forward", 0.0, 0.0, 999999.9),
			("ems_primary_l3_forward", path + "/Energy/Primary/L3/Forward", 0.0, 0.0, 999999.9),
			("ems_secondary_l1_forward", path + "/Energy/Secondary/L1/Forward", 0.0, 0.0, 999999.9),
			("ems_secondary_l2_forward", path + "/Energy/Secondary/L2/Forward", 0.0, 0.0, 999999.9),
			("ems_secondary_l3_forward", path + "/Energy/Secondary/L3/Forward", 0.0, 0.0, 999999.9),
	...
	def get_input(self):
		...
		return [
			('com.victronenergy.settings', [
				...
				'/Settings/Ems/Energy/Primary/L1/Forward',
				'/Settings/Ems/Energy/Primary/L2/Forward',
				'/Settings/Ems/Energy/Primary/L3/Forward',
				'/Settings/Ems/Energy/Secondary/L1/Forward',
				'/Settings/Ems/Energy/Secondary/L2/Forward',
				'/Settings/Ems/Energy/Secondary/L3/Forward'
			]),
	...
	def current_battery_reservation(self) -> float:
		...
		try:
			...
			# TODO: When DESS is keeping the rate lower, this has the intention to feedin. What should ems do? 
	...
	def _determine_system_type(self) -> SystemType:
		...
		self._dbusservice["/Ems/SystemType"] = system_type.value
		return system_type
	
	def _on_timer_save_counters(self):
		try:
			for l in [1,2,3]:
				self._dbusmonitor.set_value("com.victronenergy.settings", "/Settings/Ems/Energy/Primary/L{}/Forward".format(l), self.counter_primary.by_phase[l])
				self._dbusmonitor.set_value("com.victronenergy.settings", "/Settings/Ems/Energy/Secondary/L{}/Forward".format(l), self.counter_secondary.by_phase[l])

			logger_debug_proxy("Saved transient counters: P: {}/{}/{} | S: {}/{}/{}".format(
	...
	def _on_timer_track_power(self):
		try:
			...
			for l in [1,2,3]:
				...
				self._dbusservice["/Ems/PrimaryConsumer/Ac/L{}/Energy/Forward".format(l)] = self.counter_primary.by_phase[l]	
				self._dbusservice["/Ems/SecondaryConsumer/Ac/L{}/Power".format(l)] = self.power_secondary.by_phase[l]
				self._dbusservice["/Ems/SecondaryConsumer/Ac/L{}/Energy/Forward".format(l)] = self.counter_secondary.by_phase[l]

			self._dbusservice["/Ems/PrimaryConsumer/Ac/Power"] = self.power_primary.total
			self._dbusservice["/Ems/PrimaryConsumer/Ac/Energy/Forward"] = self.counter_primary.total	
			self._dbusservice["/Ems/SecondaryConsumer/Ac/Power"] = self.power_secondary.total
			self._dbusservice["/Ems/SecondaryConsumer/Ac/Energy/Forward"] = self.counter_secondary.total
	...
	def _get_available_overhead(self)-> SolarOverhead:
		...
		for unique_identifier, delegate in self.managed_rms.items():
			...
			if delegate.is_active_ems_control:
		...
		if self.system_type in [SystemType.OffGrid1Phase, SystemType.OffGrid2Phase, SystemType.OffGrid3Phase, 
						        SystemType.ZeroFeedin1Phase, SystemType.ZeroFeedin2Phase, SystemType.ZeroFeedin3Phase]:
			if self.soc is not None and self.soc >= self.balancing_threshold + 1:
				if batrate > 0 and self.dcpv_balancing_offset < 90000:
		...
		#Reset can happen independent of system type, to ensure there is no offset left over, when switching from offgrid to gridconnected system type. 
